...
const deleteAccount = async (_id, setUser) => {
   ...
   if (consent === "delete my account") {
      ...
      try {
         const { data: deleteProposal } = await axios.delete(`${process.env.REACT_APP_BACKEND_PROPOSAL}/${_id}`)
         console.log(deleteProposal)

         const { data: deleteUser } = await axios.delete(`${process.env.REACT_APP_BACKEND_USER}/${_id}`)
